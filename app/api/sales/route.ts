  ...
  try {
    ...
    await db.batch([
      db.prepare(`
        INSERT INTO sales_records
          (id, camera_id, platform, status, listed_at, sold_at, asking_price_cny,
           actual_price_cny, platform_fee_cny, shipping_cny, buyer_notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(
        ...
        numberValue(body.askingPriceCny, "挂牌价"), numberValue(body.actualPriceCny, "成交价"),
      ...
      db.prepare("UPDATE cameras SET lifecycle_status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
        .bind(status === "已出售" ? "已出售" : "待出售", cameraId),
    ]);
