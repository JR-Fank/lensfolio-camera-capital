...
export const cameras = sqliteTable("cameras", {
  ...
  lifecycleStatus: text("lifecycle_status").notNull().default("待入库"),
...
export const logisticsOrders = sqliteTable("logistics_orders", {
  ...
  allocationMethod: text("allocation_method").notNull().default("按重量"),
...
export const repairRecords = sqliteTable("repair_records", {
  ...
  valueBeforeCny: real("value_before_cny").notNull().default(0),
  valueAfterCny: real("value_after_cny").notNull().default(0),
  notes: text("notes"),
  ...timestamps,
}, (table) => [
...
export const salesRecords = sqliteTable("sales_records", {
  ...
  marketPriceCny: real("market_price_cny").notNull().default(0),
...
export const marketValuations = sqliteTable("market_valuations", {
  ...
  medianCny: real("median_cny").notNull().default(0),
  highCny: real("high_cny").notNull().default(0),
  expectedCny: real("expected_cny").notNull(),
  sampleSize: integer("sample_size"),
  conditionGrade: text("condition_grade"),
  confidence: real("confidence").notNull().default(0),
  collectionMethod: text("collection_method").notNull().default("人工录入"),
...
export const assetExpenses = sqliteTable("asset_expenses", {
  ...
  index("idx_asset_expenses_camera_date").on(table.cameraId, table.expenseDate),
]);
